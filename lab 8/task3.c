...
int main(int argc, char * argv[]){
    ...
    menu menu_opts[] = {{"Toggle Debug Mode",toggleDebugMode},{"Examine ELF File",examineElfFile},
                        ...
                        {"Relocation Tables", relocationTable},{"Quit",quit},{NULL,NULL}};
...
void relocationTable(){
    ...
    int eShnum = header->e_shnum;
    sectionHeaders = (Elf32_Shdr *)(map_start + header->e_shoff); //get a pointer to section headers
    char * sectionHeadersStringTable = (char *)(map_start + sectionHeaders[header->e_shstrndx].sh_offset);
    for(int k=0;k<eShnum;k++){
        if(sectionHeaders[k].sh_type==SHT_DYNSYM)
            sym = (Elf32_Sym *)(map_start + sectionHeaders[k].sh_offset);
        if(strcmp(sectionHeadersStringTable+sectionHeaders[k].sh_name,".dynstr")==0){
            printf("k in loop: %d\n",k);
            stringTable = (char *)(map_start + sectionHeaders[k].sh_offset);
        }
    }
    for (i = 0; i < eShnum; ++i) {
        if (sectionHeaders[i].sh_type == SHT_REL){ //if a relocation table exists
            rel = (Elf32_Rel *)(map_start + sectionHeaders[i].sh_offset); //get a pointer to relocation table
            /* Offset
            * Info*/
            printf("Relocation section %s at offset %x contains %d entries:\n",
                            sectionHeadersStringTable+sectionHeaders[i].sh_name,
                            sectionHeaders[i].sh_offset,sectionHeaders[i].sh_size/sectionHeaders[i].sh_entsize);
            printf("Offset\t\tInfo\t\ttype\t\tSym.value\tSym.name\n");
            for(j=0;j<(sectionHeaders[i].sh_size/sectionHeaders[i].sh_entsize);j++) {
                printf("%08X\t%08X\t%X\t\t%08X\t%-15s\n",
                       rel[j].r_offset, rel[j].r_info,ELF32_R_TYPE(rel[j].r_info),
                       sym[ELF32_R_SYM(rel[j].r_info)].st_value,stringTable + sym[ELF32_R_SYM(rel[j].r_info)].st_name);
            }
        }
    }
}
