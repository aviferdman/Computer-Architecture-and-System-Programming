...
void freeLinkedList(Link* linked_list){
    if(linked_list!=NULL){
        if(linked_list->next!=NULL){
            freeLinkedList(linked_list->next);
        }
        free(linked_list);
    }
}
...
void executePipedCommand(cmdLine * line){
    pid_t pid1,pid2;
    ...
    if(pid1 ==0){
        ...
        execvp(line->arguments[0],line->arguments); //execute
    }
