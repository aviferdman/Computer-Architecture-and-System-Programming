...
void detect_virus(char *buffer, unsigned int size, link *virus_list){
    ...
    while (virus_list!=NULL) {
        while (index <= size - virus_list->vir->SigSize) {
            cmp = memcmp(virus_list->vir->sig, buffer+index, virus_list->vir->SigSize);
            if (cmp == 0) {
...
int main(int argc, char **argv) {
    ...
    FILE* file;
    link* virusList = NULL;
    struct fun_desc menu[5] = { { "Load signatures", loadSignatures }, { "Print signatures",  list_print }, { "Detect viruses", detect_virus },
                                { "Quit", NULL },{ NULL, NULL }};
