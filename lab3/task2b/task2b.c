    ...
    while (virus_list) {
        while (index <= size - virus_list->vir->SigSize) {
            if (!memcmp(virus_list->vir->sig, buffer+index, virus_list->vir->SigSize)) {
                printf("Starting byte location: %d\nThe virus name is: %s\nThe size of virus signature is: %d",index,
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    char nulls[signitureSize];
    int i;
    for(i=0; i<signitureSize; i=i+1){
        nulls[i] = 0x90;
    }
    FILE *f = fopen(fileName, "r+");
    fseek(f, signitureOffset, SEEK_SET);
    fwrite(nulls, 1, signitureSize, f);
    fclose(f);
}
...
int main(int argc, char **argv) {
    ...
    int bounds = 0, chosenOption = 0, fileSize, virusBegin =0, virusSize =0;
    unsigned int min = 0;
    FILE* file;
    link* virusList = NULL;
    struct fun_desc menu[6] = { { "Load signatures", loadSignatures }, { "Print signatures",  list_print },
                                { "Detect viruses", detect_virus },{ "Fix file", kill_virus },
                                { "Quit", NULL },{ NULL, NULL }};
    ...
    while (chosenOption<bounds && chosenOption >=0) //in bounds
    {
        if (chosenOption == 1) {
            getchar();
            gets(fileName);
            list_free(virusList);
            menu[chosenOption - 1].fun(fileName, &virusList);
        }
        else if (chosenOption == 2){
            menu[chosenOption - 1].fun(virusList, stdout);
        }
            ...
            menu[chosenOption - 1].fun(buffer, fileSize , virusList);
            fclose(file);
        }
            ...
            printf("\nsize of virus: ");
            scanf("%d", &virusSize);
            menu[chosenOption - 1].fun(argv[1], virusBegin, virusSize);
        }
